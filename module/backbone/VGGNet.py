...
class VGG16(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.relu(out)
        out1 = self.maxpool1(out)  # 112

        out = self.conv2_1(out1)  # 110
        out = F.relu(out)
        out = self.conv2_2(out)  # 110
        out = F.relu(out)
        out2 = self.maxpool2(out)  # 56

        out = self.conv3_1(out2)  # 54
        out = F.relu(out)
        out = self.conv3_2(out)  # 54
        out = F.relu(out)
        out = self.conv3_3(out)  # 54
        out = F.relu(out)
        out3 = self.maxpool3(out)  # 28

        out = self.conv4_1(out3)  # 26
        out = F.relu(out)
        out = self.conv4_2(out)  # 26
        out = F.relu(out)
        out = self.conv4_3(out)  # 26
        out = F.relu(out)
        out4 = self.maxpool4(out)  # 14

        out = self.conv5_1(out4)  # 12
        out = F.relu(out)
        out = self.conv5_2(out)  # 12
        out = F.relu(out)
        out = self.conv5_3(out)  # 12
        out = F.relu(out)
        out5 = self.maxpool5(out)  # 7
